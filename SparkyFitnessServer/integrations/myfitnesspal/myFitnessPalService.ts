...
export async function pushNutritionToMFP(
  ...
  try {
    ...
    try {
        const csrfResp = await axios.get('https://www.myfitnesspal.com/api/auth/csrf', {
        ...
        log('warn', `pushNutritionToMFP: CSRF fetch fallback for user ${userId}: ${e.message}`);
    }
    ...
    let mfpUserId = '';
    try {
        const authResp = await axios.get('https://www.myfitnesspal.com/user/auth_token', {
            headers: { 
                ...baseHeaders, 
        ...
        log('error', `pushNutritionToMFP: Auth failed for user ${userId}: ${e.message}`);
    ...
    try {
        ...
        try {
            ...
            log('error', `pushNutritionToMFP: Primary discovery failed with ${e.response?.status}: ${JSON.stringify(e.response?.data || e.message)}`);
            // Fallback to minimal discovery
            diaryResp = await axios.get(`https://api.myfitnesspal.com/v2/diary?entry_date=${data.date}`, {
                headers: authHeaders as any
            });
        }
        ...
        if (diaryResp.data && Array.isArray(diaryResp.data.items)) {
            ...
            log('info', `pushNutritionToMFP: Discovery returned ${items.length} items for ${data.date}.`);
        ...
        log('warn', `pushNutritionToMFP: Idempotency cleanup failed for user ${userId}: ${e.message}`);
    }
    ...
    log('error', `pushNutritionToMFP: Final fatal error for user ${userId}:`, error.response?.data || error.message);
    throw error;
  }
}
