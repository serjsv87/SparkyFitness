...
import { syncDailyNutritionToMFP } from '../services/mfpSyncService.js';
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (daysDiff > 31) {
        return res
          .status(400)
          .json({ error: 'Date range cannot exceed 31 days.' });
      }
      ...
      log(
        'info',
        `mfpRoutes: Manual MFP sync requested for user ${userId} for ${dates.length} days`
      );

      // Trigger syncs sequentially to be safe with locking and rate limits
      for (const date of dates) {
        await syncDailyNutritionToMFP(userId, date);
      }
