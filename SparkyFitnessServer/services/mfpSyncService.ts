 ...
 */
export async function syncDailyTotals(userId: string, date: string): Promise<any> {
    const lockKey = `${userId}:${date}`;
    if (activeSyncs.has(lockKey)) {
        log('info', `mfpSyncService: Sync already in progress for user ${userId} on ${date}. Skipping concurrent request.`);
    ...
    try {
        log('info', `mfpSyncService: Starting categorized sync for user ${userId} on ${date}`);
        ...
        if (!categories || Object.keys(categories).length === 0) {
            log('info', `mfpSyncService: No nutrition data found for ${userId} on ${date}. Sync skipped.`);
        ...
        log('info', `mfpSyncService: Successfully synced categorized totals for user ${userId} on ${date}`);
        return { status: 'success', date, ...result };

    } catch (error: any) {
        log('error', `mfpSyncService: Failed to sync totals for user ${userId} on ${date}:`, error.message);
